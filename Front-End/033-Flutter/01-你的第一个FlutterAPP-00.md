...
- 在*RandomWordsState*类中添加*_buildSuggestions()*函数，这个函数是在*ListView*中显示建议的词对。
  ...
  *ListView*提供**itemBuilder**这个builder属性。
  ...
  1. 每个建议的词对调一次*itemBuilder*回调，会将这个建议的值放置到*ListTile*中。偶数行时，回调函数为词对值添加一个*ListTile*。奇数行时，函数添加一个**Divider**组件来直观上的区分。在较小的设备上可能很难看到这个分隔符。
  2. 在**ListView**的每一行前，添加一像素高的分隔符组件。
  3. 表达式：i ~/ 2，得到一个整数。这个整数*ListView*中去掉分隔符组件后词对的的实际数量。
  4. 当到达词对列表底部的时候，再增加十个词对到列表中。
